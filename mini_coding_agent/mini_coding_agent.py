...
class OpenAIModelClient:
    def __init__(self, model, api_key, temperature, top_p, timeout):
        self.model = model
...
def build_agent(args):
    ...
    if not api_key:
        raise RuntimeError("No OpenAI API key found. Set OPENAI_API_KEY in .env or pass --api-key.")
...
def build_arg_parser():
    ...
    parser.add_argument("--api-key", default="", help="OpenAI API key (falls back to OPENAI_API_KEY env var).")
    parser.add_argument("--timeout", type=int, default=120, help="OpenAI request timeout in seconds.")
