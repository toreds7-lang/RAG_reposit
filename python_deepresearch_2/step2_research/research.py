...
def generate_serp_queries(
    ...
    response_json = JSON_llm(prompt, SerpQueryResponse, system_prompt=sys_prompt, model=model)
    try:
...
def process_serp_result(
    ...
    sys_prompt = system_prompt()
    response_json = JSON_llm(prompt, SerpResultResponse, system_prompt=sys_prompt, model=model)
    try:
...
def deep_research(
    ...
    serp_queries = generate_serp_queries(query=query, model=model, num_queries=breadth, learnings=learnings)
